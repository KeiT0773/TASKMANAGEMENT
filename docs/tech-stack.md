...
| フレームワーク | Spring Boot（最新の安定版） | プロジェクト方針として指定。具体的なバージョンは雛形作成時に Spring Initializr で確認し、本書に追記する |
| ビルドツール | Gradle | ソースのコンパイル、依存ライブラリの取得、実行ファイルの作成を行う |
...
| 項目 | 選定 | 役割・選定理由 |
| --- | --- | --- |
| ライブラリ | React | プロジェクト方針として指定 |
| 言語 | TypeScript | API がやり取りするデータ（カード・リスト）の形を型として定義でき、バックエンドとの食い違いを早期に検出できる |
| ビルド・開発サーバー | Vite | Next.js を使わない場合の標準的な選択。起動が速く、初期設定がほぼ不要 |
...
| コード検査・整形 | ESLint + Prettier | Vite の雛形に ESLint が同梱される |
| テスト | Vitest + React Testing Library | Vite と相性のよい標準的な組み合わせ |
| パッケージ管理 | npm | Node.js に同梱。追加インストール不要 |
...
| 必要なインストール | JDK 21、Node.js（LTS）、Docker Desktop | 実装開始前に導入する |
...
| Spring Boot の具体的なバージョン | 雛形作成時点の最新安定版を採用する | 雛形作成時 |
