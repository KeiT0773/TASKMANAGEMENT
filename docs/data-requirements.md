...
本書は、本システムがデータベースに保持すべき情報と、その構造・流れを定義する。具体的な格納形式（テーブル定義、型、桁数）は基本設計で決定する。各項目に関連する機能（FR-xx）の内容は [機能要件書](functional-requirements.md) を、画面（SC-xx）は [画面要件書](screen-requirements.md) を参照すること。
