...
| 1.3 | 2026-09-21 | FR-10「全リストの優先度順並べ替え」の追加に伴い、「表示順」の変化要因とデータフロー D1 に追記 | KeiT0773 |
...
| 表示順 | 数値 | システム | 必須 | リスト内での並び順。ドラッグによる並べ替えのほか、カード登録・優先度変更後の優先度順並べ替え、全リストの優先度順並べ替えでも変化する（FR-01, FR-05, FR-08, FR-10） |
...
| No | 流れ | 内容 | 関連機能 |
| --- | --- | --- | --- |
| D1 | 利用者 → ボード画面 | カードの登録、列の間の移動、列内の並べ替え、全リストの優先度順並べ替え | FR-01, FR-04, FR-05, FR-10 |
