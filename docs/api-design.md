...
1.x ではカードとリストの **取得（READ）** のみを定義していた。本版（2.0）では **カードの登録（FR-01）** を追加する。更新・削除・移動・並べ替えは、それぞれの実装に着手するときに本書へ追記する（10. 保留事項）。
...
| 404 Not Found | 対象が存在しない | パスで指定した `id` のカードが無い |
...
更新・削除・移動・並べ替え（FR-02〜FR-08）は 10. 保留事項のとおり、実装時に追記する。
...
| 11 | body で指定した `listId` のリストが存在しないときは 404 ではなく 400 とする | 8. | 2026-09-21 |
...
| 事項 | 保留理由 | 決定時期 |
| --- | --- | --- |
| カードの更新・削除・移動・並べ替えの API（FR-02〜FR-08） | 登録 API（8.）を先に完成させ、書き込みの一連の流れをフロントエンドまで通してから着手する | 各機能の実装着手時に本書へ追記 |
| 優先度を Java の列挙型（enum）で扱うか | 登録 API では文字列のまま扱い、入力チェックで 3 値に限定する。更新 API で優先度の変更（FR-08）が入ると、並べ替えの比較に使う箇所が増える | 更新 API の追記時 |
| API 仕様書ツール（springdoc-openapi / Swagger UI）の導入 | [技術スタック](tech-stack.md) 3. で選定済みだが未導入。登録 API の時点では API が 4 本と少なく、本書を正として手で保守できる | 更新・削除 API が揃い、API が増えたとき |
