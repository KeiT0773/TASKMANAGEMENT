...
| 2.3 | 2026-09-21 | FR-10「全リストの優先度順並べ替え」を追加。2.2 課題 No.3、5.2 利用シナリオ、6.・6.1 の機能一覧（10 機能）、決定事項 No.12、受け入れ基準 11.1 を更新 | KeiT0773 |
...
| 3 | 何から手を付けるべきかが、メモを読み返さないと判断できない | カードに優先度と期限を表示し、一覧を見ただけで着手順を判断できるようにする。ボタン 1 つで全リストを優先度順に並べ直せるようにする | FR-07, FR-08, FR-10 |
...
| 〃 | 「優先度順に並べ替え」を押して、3 つの列を優先度の高い順にそろえる | 3 つの列それぞれで、優先度の高いカードが上に来る。同じ優先度の中の順序は変わらない | SC-01 | FR-10 |
...
利用者がカードに対して行う操作（登録・編集・削除・移動・並べ替え）、カードに付与する情報（説明文・期限・優先度）、サーバーへのデータ保存、および全リストの優先度順並べ替えの10機能で構成する。すべて必須機能とする。
...
| 12 | すべてのリストを一括で優先度順（高→中→低）に並べ直すボタンを、リストの枠の外（ヘッダーとリストの間）に 1 つ置く。同じ優先度の中では既存の順序を保つ。操作前の確認は求めず、元に戻す手段は提供しない（ドラッグで整え直せる） | FR-10 | 2026-09-21 |
...
| FR-10 | 3 つの列それぞれで「低」のカードを「高」のカードより上にドラッグしてから、「優先度順に並べ替え」を押す | 3 つの列すべてが高→中→低の順になる。同じ優先度のカードどうしの順序は押す前と同じ。ページを再読み込みしても維持される |
| FR-10 | すべてのカードが同じ優先度の列で「優先度順に並べ替え」を押す | その列の順序は変わらない |
