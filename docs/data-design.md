...
| 1.3 | 2026-09-21 | FR-10「全リストの優先度順並べ替え」の追加に伴い、6. の振り直しを行う操作・振り直しの単位に追記し、決定事項 No.11 を追加 | KeiT0773 |
...
| 振り直しを行う操作 | カードの登録（FR-01）、削除（FR-03）、リスト間の移動（FR-04）、リスト内の並べ替え（FR-05）、優先度の変更（FR-08）、全リストの優先度順並べ替え（FR-10） |
| 振り直しの単位 | 影響を受けたリストのカード全件。リスト間の移動では、移動元と移動先の2つのリストが対象になる。全リストの優先度順並べ替え（FR-10）では3つのリストすべてが対象になる |
...
優先度順に並べ直す規則そのもの（高 → 中 → 低、同じ優先度の中では既存の順序を保つ、登録・優先度変更では操作したカードをそのグループの末尾に置く）は [機能要件書](functional-requirements.md) 3.1 に定義されている。バックエンドはその規則に従って新しい並びを決め、その結果を 0 からの通し番号として書き込む。
...
| 11 | 全リストの優先度順並べ替え（FR-10）も、3つのリスト分の振り直しを1つのトランザクションにまとめて行う。途中で失敗した場合は3つとも操作前の状態に戻す | 6. | 2026-09-21 |
