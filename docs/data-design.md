...
| REST API の一覧、要求・応答の形式 | 本書では扱わない | 別途作成する |
...
`cards` には初期データを投入しない。アプリを初めて開いたときは、3つの空の列が表示される状態から始まる。
...
| REST API の一覧と、要求・応答の JSON 形式 | フロントエンドとの取り決めであり、変更頻度が高いため別文書に分ける | API 実装の着手時 |
